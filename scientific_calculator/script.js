...
// Shared converters used by the Base and Unit tabs.
...
function formatResultNumber(value) {
    ...
    if ((absValue !== 0 && absValue < 0.000001) || absValue >= 1000000) {
...
function updateUnitOptions() {
    ...
    const config = UNIT_DEFINITIONS[typeSelect.value] || UNIT_DEFINITIONS.length;
    const previousFrom = fromSelect.value;
    const previousTo = toSelect.value;
    const optionsMarkup = config.units.map(unit =>
    ...
    const hasPreviousFrom = config.units.some(unit => unit.value === previousFrom);
    const hasPreviousTo = config.units.some(unit => unit.value === previousTo);
    fromSelect.value = hasPreviousFrom ? previousFrom : config.units[0].value;
    toSelect.value = hasPreviousTo ? previousTo : (config.units[1] ? config.units[1].value : config.units[0].value);
}

function convertBase() {
    const inputEl = document.getElementById('base-input');
    const fromEl = document.getElementById('base-from');
    const toEl = document.getElementById('base-to');
    ...
    if (!rawValue) {
        setResultText('base-result', 'Result: Enter a number first');
        return;
    }
    ...
    if (!validators[fromBase] || !validators[fromBase].test(unsignedValue)) {
        setResultText('base-result', `Result: Invalid base-${fromBase} number`);
        return;
    }
    ...
    if (fromBase === 10) {
        ...
        decimalValue = parseInt(unsignedValue, fromBase) * sign;
    }

    if (!Number.isFinite(decimalValue)) {
        setResultText('base-result', 'Result: Conversion failed');
        return;
    }

    if (toBase !== 10 && !Number.isInteger(decimalValue)) {
        setResultText('base-result', 'Result: Non-integers can only be shown in decimal');
        return;
    }

    let result;
    if (toBase === 10) {
        result = formatResultNumber(decimalValue);
    } else {
        const absValue = Math.abs(decimalValue);
        result = absValue.toString(toBase).toUpperCase();
        if (decimalValue < 0) {
            result = `-${result}`;
        }
    }

    setResultText('base-result', `Result: ${result}`);
}
...
function convertTemperature(value, fromUnit, toUnit) {
    ...
    if (fromUnit === 'F') celsiusValue = (value - 32) * 5 / 9;
    if (fromUnit === 'K') celsiusValue = value - 273.15;

    if (toUnit === 'F') return (celsiusValue * 9 / 5) + 32;
    if (toUnit === 'K') return celsiusValue + 273.15;
    return celsiusValue;
}
...
function convertUnit() {
    ...
    if (!inputEl || !typeEl || !fromEl || !toEl) return;
    ...
    if (!rawValue || Number.isNaN(numericValue)) {
        setResultText('unit-result', 'Result: Enter a valid number');
        return;
    }

    const config = UNIT_DEFINITIONS[type];
    if (!config) {
        setResultText('unit-result', 'Result: Unsupported unit type');
        return;
    }
    ...
    if (type === 'temperature') {
        ...
        const fromConfig = config.units.find(unit => unit.value === fromUnit);
        const toConfig = config.units.find(unit => unit.value === toUnit);
        if (!fromConfig || !toConfig) {
            setResultText('unit-result', 'Result: Select valid units');
            return;
        }

        const baseValue = numericValue * fromConfig.factor;
        result = baseValue / toConfig.factor;
    }

    const targetUnitLabel = config.units.find(unit => unit.value === toUnit)?.value || toUnit;
    setResultText('unit-result', `Result: ${formatResultNumber(result)} ${targetUnitLabel}`);
}
...
function analyzeFunction() {
    ...
    let isCausal = true;
    let isStable = true;
    let stabilityReason = 'bounded for standard bounded inputs';

    const lowerExpr = expression.toLowerCase().replace(/\s+/g, '');

    if (/1\/x/.test(lowerExpr)) {
        domain = 'x != 0';
        isStable = false;
        stabilityReason = 'singularity at x = 0';
    } else if (/sqrt\(/.test(lowerExpr)) {
        domain = 'x >= 0';
    } else if (/log\(|log10\(|ln\(/.test(lowerExpr)) {
        domain = 'x > 0';
        isStable = false;
        stabilityReason = 'undefined for non-positive x';
    } else if (/tan\(/.test(lowerExpr)) {
        domain = 'x != pi/2 + k*pi';
        isStable = false;
        stabilityReason = 'tan(x) blows up at odd pi/2';
    }

    if (/\^|sin|cos|tan|log|ln|sqrt|abs/.test(lowerExpr)) {
        isLinear = false;
    }

    if (/y\(t[+-]|\bt\b/.test(lowerExpr) || /x\(t[+-]/.test(lowerExpr) || /x\(n[+-]/.test(lowerExpr)) {
        isTimeInvariant = false;
    }

    if (/integral|deriv|sum|delay|x\(t[-+]|x\(n[-+]/.test(lowerExpr)) {
        isMemoryless = false;
    }

    if (/x\(t\+|x\(n\+|y\(t\+|y\(n\+/.test(lowerExpr)) {
        isCausal = false;
    }
    ...
    linearOut.innerText = isLinear ? 'Linear' : 'Non-Linear';
    timeOut.innerText = isTimeInvariant ? 'Time Invariant' : 'Time Variant';
    memoryOut.innerText = isMemoryless ? 'Memoryless' : 'Has Memory';
    causalOut.innerText = isCausal ? 'Causal' : 'Non-Causal';
    stableOut.innerText = isStable ? `Stable (${stabilityReason})` : `Unstable (${stabilityReason})`;
}
