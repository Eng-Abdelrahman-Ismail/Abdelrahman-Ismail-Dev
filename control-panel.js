(() => {
    ...
    const TEXT = {
        ar: {
            ...
            loginHint: 'سجل دخول الأدمن للوصول إلى لوحة التحكم',
            loginUserLabel: 'اسم المستخدم أو الإيميل',
            loginPassLabel: 'كلمة المرور',
            loginUserPlaceholder: 'اسم المستخدم أو الإيميل',
            loginPassPlaceholder: 'كلمة المرور',
            loginButton: 'دخول',
            loginError: 'خطأ في تسجيل الدخول: تأكد من الإيميل والباسورد',
            loginUnavailable: 'تعذر تحميل نظام تسجيل الدخول. أعد تحميل الصفحة.',
            firebaseManagedAuth: 'تسجيل دخول الأدمن أصبح عبر Firebase. غيّر كلمة المرور من Firebase Console.',
        ...
        en: {
            ...
            loginHint: 'Sign in to access the admin dashboard',
            loginUserLabel: 'Username or Email',
            loginPassLabel: 'Password',
            loginUserPlaceholder: 'Username or Email',
            loginPassPlaceholder: 'Password',
            loginButton: 'Login',
            loginError: 'Login failed: check the email and password',
            loginUnavailable: 'Authentication module failed to load. Refresh the page.',
            firebaseManagedAuth: 'Admin sign-in now uses Firebase. Change the password from Firebase Console.',
    ...
    function init() {
        ...
        handleSecretAdminAccessFromLocation();
    }

    function cacheElements() {
        els.launcher = document.getElementById('cpLauncher');
        els.logo = document.querySelector('.logo');
        ...
        els.username = document.getElementById('adminEmail') || document.getElementById('cpUsername');
        els.password = document.getElementById('adminPassword') || document.getElementById('cpPassword');
        els.loginBtn = document.getElementById('loginBtn') || document.getElementById('cpLoginBtn');
    ...
    function bindEvents() {
        ...
        document.addEventListener('keydown', (event) => {
            ...
            const isHiddenAdminShortcut = event.ctrlKey && event.shiftKey && (
                event.code === 'KeyA' ||
                event.code === 'KeyM' ||
                key === 'a' ||
                key === 'm'
        ...
        if (els.logo) {
            els.logo.addEventListener('click', handleSecretLogoClick);
        }

        window.addEventListener('hashchange', handleSecretAdminAccessFromLocation);
    ...
    function handleLogin(event) {
        ...
        if (window.portfolioAdminAuth && typeof window.portfolioAdminAuth.submitLogin === 'function') {
            window.portfolioAdminAuth.submitLogin(event);
            return;
        }

        els.loginError.textContent = t('loginUnavailable');
    }

    function handleFirebaseAuthSuccess() {
    ...
    function handleFirebaseAuthFailure(message, error) {
        if (els.loginError) {
            els.loginError.textContent = message || t('loginError');
        }
        if (error) {
    ...
    function syncFirebaseAuthState(user) {
        if (user) {
            isAdminAuthenticated = true;
            setLauncherVisibility(true);
            sessionStorage.setItem(SESSION_KEY, '1');
    ...
    function exposeAdminBridge() {
        window.portfolioAdminPanel = {
            handleAuthSuccess: handleFirebaseAuthSuccess,
            handleAuthFailure: handleFirebaseAuthFailure,
            syncAuthState: syncFirebaseAuthState,
            logout,
            openPanel,
            closePanel,
            openLogin,
    ...
    function setLauncherVisibility(isVisible) {
        if (!els.launcher) {
            return;
        }

        els.launcher.hidden = !isVisible;
    ...
    function handleSecretAdminAccessFromLocation() {
        const search = new URLSearchParams(window.location.search);
        const hasSecretAdminFlag = window.location.hash === '#admin' || search.get('admin') === '1';

        if (!hasSecretAdminFlag) {
            return;
        }
    ...
    function logout() {
        ...
        setMode('none');
        closePanel();
        if (window.portfolioAdminAuth && typeof window.portfolioAdminAuth.signOut === 'function') {
            window.portfolioAdminAuth.signOut().catch((error) => {
                console.error(error.message || error);
            });
        }
    ...
    function updatePasscode() {
        ...
        showStatus(t('firebaseManagedAuth'));
    }
